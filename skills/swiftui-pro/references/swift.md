...
- If an API offers both modern `async`/`await` equivalents and older closure-based variants, prefer `async`/`await` when their semantics and performance characteristics fit the work.
- Prefer Swift concurrency (`async`/`await`, actors, `Task`) for new application code. GCD remains appropriate for some framework interoperation, real-time callbacks, and measured low-level synchronization; review those uses for correctness rather than rejecting them categorically.
